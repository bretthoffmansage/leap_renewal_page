# ForgeShell — Claude runtime instructions

## Purpose

This file tells Claude Code how to operate inside the **ForgeShell** reusable autonomous shell (**`shell-core`** working tree).

This repo is not a normal app repo.
It is a reusable shell system.

Claude must treat it as a workflow engine that moves a project from:
- raw PRD
to:
- planning docs
- execution packages
...
### Safe git-init command pattern (Claude-specific)

When local git initialization is needed, assume the Claude session is already started at the shell project root.

Do **not** use compound shell commands like:
- `cd <path> && git init`
- `cd <path> && git init && git status`

Do **not** combine `cd` and `git` in one Bash invocation for shell startup checks.

Instead:
1. confirm the current working directory is already the shell project root
2. run `git init` as its own command only if `.git` is absent
3. run `git status` as a separate command after init if needed

If Claude is not already at the intended shell root, first acknowledge that explicitly and ask to restart in the correct directory or use the safest available non-compound path.

Reason:
Claude Code may require extra approval for compound `cd` + `git` command patterns, especially around repository initialization and `.git` writes.
...
After **final closeout** is complete and **only** if `.forgeshell/packaging.json` exists (bootstrap: PRD path, **then** application name), follow **`shell/reusable/REUSABLE_SHELL_WORKFLOW.md` — Phase I** and **`shell/reusable/REUSABLE_SHELL_PROMPTS.md`** (**#16** / **#17**).

Offer exactly **two** choices (no automatic cleanup): **Clean for handoff** vs **Leave workspace as-is** — use the exact **Leave workspace as-is** wording from the workflow doc: **Keep the finished application inside the full ForgeShell workspace with all shell files intact**.

**Clean for handoff** uses `node scripts/forgeshell-packaging-handoff.mjs --mode clean` from **`shell-core`** after the operator records `choice: clean-for-handoff`; it archives into **`_forgeshell_archive/`**, removes shell control trees, rewrites app **`README.md`**, trims shell-only **`.gitignore`**, renames **only** `shell-core`, and archives ForgeShell-only **`scripts/`** (or only the packaging script when **`scripts/`** is mixed).
